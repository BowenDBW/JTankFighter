    ...
    public ClientController(ClientView thisView) {
        ...
        view.getSendMessage().addActionListener(e -> {
                    if (!Status.isGameStarted()) {
                        DrawingPanel.addMessage("还没有和主机端玩家联上, 无法发送对话");
                        return;
                    }

                    if (!"".equals(view.getMessageField().getText())) {
                        DrawingPanel.addMessage("用户端玩家说：" +
        ...
        view.getHelp().addActionListener(e -> {
            DrawingPanel.addMessage("******************************坦克大战 ******************************");
                    DrawingPanel.addMessage("帮助: 按s键发射子弹,按键盘的方向键来控制坦克的移动");
                    DrawingPanel.addMessage("如果按键没有反应请 1. 关闭大写功能; 2. 用 tab键切换 ");
                    DrawingPanel.addMessage("如果您在使用对话界面请移动到控制界面.");
        ...
        temp.addKeyListener(new KeyAdapter() {
                    ...
                    public void keyPressed(KeyEvent e) {
                        ...
                        if (e.getKeyChar() == 'y' && Status.isGameOver()
                            ...
                            DrawingPanel.addMessage("等待主机端玩家回应...");
                        }
